...
export class GestionGroupesComponent implements OnInit {
  ...
  constructor(
    ...
    private gestionMatieresService: GestionMatieresService,
    private gestionSpecialitesService: GestionSpecialitesService,
    private gestionGroupesService: GestionGroupesService
  ) {}

  async ngOnInit() {
    await this.gestionSpecialitesService
      .getAllSpecialites()
      .toPromise()
      .then((results) => {
        this.specialites = results;
      });
    await this.gestionMatieresService
      .getAllMatieres()
      .toPromise()
      .then((data) => {
        this.matieres = data;
      });
    await this.gestionGroupesService
      .getAllGroupes()
      .toPromise()
      .then((results) => {
        ...
        if (this.groupes.length <= 5) {
          ...
          if (this.groupes.length % 5 == 0) {
            ...
            this.pageItems = new Array(Math.trunc(this.groupes.length / 5) + 1);
          }
        }
        this.currentItem = 0;
  ...
  openAjout(content, type, modalDimension) {
    if (modalDimension === "lg" && type === "modal_mini") {
      ...
      this.modalService
        .open(content, { backdrop: false, centered: true })
        .result.then(
          (result) => {
            this.closeResult = "Closed with: $result";
          },
  ...
  openAjoutMat(content, type, modalDimension, groupe) {
    this.groupeMa = groupe;
    console.log(this.groupeMa,'grpppppp');
    if (modalDimension === "lg" && type === "modal_mini") {
      this.modalService
        .open(content, {
          backdrop: false,
  ...
  add_groupe() {
    ...
    console.log(g);
    let resp = this.gestionGroupesService
      .addGroupe(g, this.addFormGroupe.get("spec").value)
      .subscribe();
  }

  successSwalAjout() {
    Swal.fire("Ce groupe est ajouté!", "", "success").then(function () {
  ...
  add_MatToGrp() {
    // console.log(this.addFormMatToGrp.get("nomMat").value,'tttttttttt');
    let resp = this.gestionGroupesService
      .addMatToGroupe(
        this.addFormMatToGrp.get("nomMat").value,
        this.groupeMa.id
      )
      .subscribe(()=>{
        this.successSwalAjoutmat();
      });
